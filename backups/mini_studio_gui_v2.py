...
# =========================
def default_notes():
    return f"""# 🎙 Mini Studio Session Notes
...
def save_notes(output_dir, content: str):
    path = Path(output_dir) / NOTES_FILE
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)


def load_notes(output_dir):
    path = Path(output_dir) / NOTES_FILE
    if not path.exists():
        content = default_notes()
        save_notes(output_dir, content)
        return content
...
# 🎧 BLOCK CONCAT
# =========================
def save_block(block_segments, out_dir: Path, block_index: int, sr: int):
    block_audio = torch.cat([s.audio for s in block_segments], dim=0)
    path = out_dir / f"block_{block_index:03d}.wav"
...
def run_generation(
    ...
    session_state = {
        ...
        "love_my_mac": love_mac,
        "cool_every": cool_every,
        "cool_seconds": cool_seconds,
        "last_run": datetime.now().isoformat()
    }

    save_session(out_dir, session_state)
    ...
    for i, utt in enumerate(conversation):
        ...
        try:
            ...
            # ---- Save per-turn
            turn_path = out_dir / f"turn_{i+1:04d}_speaker_{utt['speaker_id']}.wav"
            torchaudio.save(str(turn_path), audio.unsqueeze(0), generator.sample_rate)

            # ---- Block save
            if len(block_segments) >= block_size:
                last_block_path = save_block(block_segments, out_dir, block_index, generator.sample_rate)
    ...
    if generated_segments:
        ...
        final_path = out_dir / "FULL_CONVERSATION.wav"
...
def launch_gui():
    ...
    demo.launch(server_port=7866, share=False)
